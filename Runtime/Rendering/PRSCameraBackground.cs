...
{
  /// <remarks>
  /// Based on <see cref="UnityEngine.XR.ARFoundation.ARCameraBackground"/>.
  /// </remarks>
  ...
  public class PRSCameraBackground : MonoBehaviour
  {
    ...
    /// </summary>
    const string k_CustomRenderPassName = "PRS Background Pass (LegacyRP)";
    ...
    protected bool shouldInvertCulling => m_CameraManager != null && (m_CameraManager.subsystem?.invertCulling ?? false);
    ...
    private static readonly int MainTex = Shader.PropertyToID(k_MainTexName);
    ...
    {
      Texture texture = !material.HasProperty(k_MainTexName) ? null : material.GetTexture(MainTex);
...
//#endif
    ...
    void SetMaterialKeywords(Material mat, List<string> enabledMaterialKeywords,
      List<string> disabledMaterialKeywords)
    {
      if (enabledMaterialKeywords != null)
      {
        ...
        {
          if (!mat.IsKeywordEnabled(materialKeyword))
          {
            mat.EnableKeyword(materialKeyword);
          }
        }
      }
      ...
      if (disabledMaterialKeywords != null)
      {
        foreach (var materialKeyword in disabledMaterialKeywords)
        {
          if (mat.IsKeywordEnabled(materialKeyword))
          {
            mat.DisableKeyword(materialKeyword);
          }
        }
      }
    }
